...
func testUpdateRenewProjectForecast(t *testing.T, c *TestContext, ID int) {
	...
	for i, tc := range tcc {
		...
		for _, r := range tc.RespContains {
			if !strings.Contains(body, r) {
				t.Errorf("UpdateRenewProjectForecast[%d]\n  ->attendu %s\n  ->reçu: %s", i, r, body)
			}
		}
		status := response.Raw().StatusCode
		if status != tc.StatusCode {
			t.Errorf("UpdateRenewProjectForecast[%d]  ->status attendu %d  ->reçu: %d", i, tc.StatusCode, status)
		}
	}
}
...
func testGetRenewProjectForecast(t *testing.T, c *TestContext, ID int) {
	...
	for i, tc := range tcc {
		...
		for _, r := range tc.RespContains {
			if !strings.Contains(body, r) {
				t.Errorf("GetRenewProjectForecast[%d]\n  ->attendu %s\n  ->reçu: %s", i, r, body)
			}
		}
		status := response.Raw().StatusCode
		if status != tc.StatusCode {
			t.Errorf("GetRenewProjectForecast[%d]  ->status attendu %d  ->reçu: %d", i, tc.StatusCode, status)
		}
	}
}
...
func testGetRenewProjectForecasts(t *testing.T, c *TestContext, ID int) {
	...
	for i, tc := range tcc {
		...
		for _, r := range tc.RespContains {
			if !strings.Contains(body, r) {
				t.Errorf("GetRenewProjectForecasts[%d]\n  ->attendu %s\n  ->reçu: %s", i, r, body)
			}
		}
		status := response.Raw().StatusCode
		if status != tc.StatusCode {
			t.Errorf("GetRenewProjectForecasts[%d]  ->status attendu %d  ->reçu: %d", i, tc.StatusCode, status)
		}
		if status == http.StatusOK {
			count := strings.Count(body, `"ID"`)
			if count != tc.Count {
				t.Errorf("GetRenewProjectForecasts[%d]  ->nombre attendu %d  ->reçu: %d", i, tc.Count, count)
			}
		}
	}
}
...
func testDeleteRenewProjectForecast(t *testing.T, c *TestContext, ID int) {
	...
	for i, tc := range tcc {
		...
		for _, r := range tc.RespContains {
			if !strings.Contains(body, r) {
				t.Errorf("DeleteRenewProjectForecast[%d]\n  ->attendu %s\n  ->reçu: %s", i, r, body)
			}
		}
		status := response.Raw().StatusCode
		if status != tc.StatusCode {
			t.Errorf("DeleteRenewProjectForecast[%d]  ->status attendu %d  ->reçu: %d", i, tc.StatusCode, status)
		}
	}
}
