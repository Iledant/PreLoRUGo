...
func (r *RenewProjectForecast) Create(db *sql.DB) (err error) {
	...
	(commission_id,value,project,comment,renew_project_id,action_id)
 VALUES($1,$2,$3,$4,$5,$6) RETURNING id`, &r.CommissionID, &r.Value, &r.Project,
		&r.Comment, &r.RenewProjectID, &r.ActionID).Scan(&r.ID)
	if err != nil {
		return err
	}
...
func (r *RenewProjectForecast) Get(db *sql.DB) (err error) {
	err = db.QueryRow(`SELECT r.commission_id,c.date,c.name,r.value,r.project,
	...
	WHERE r.id=$1`, r.ID).Scan(&r.CommissionID, &r.CommissionDate, &r.CommissionName,
		&r.Value, &r.Project, &r.Comment, &r.RenewProjectID, &r.ActionID, &r.ActionCode,
		&r.ActionName)
	if err != nil {
		return err
	}
...
func (r *RenewProjectForecast) Update(db *sql.DB) (err error) {
	...
	comment=$3,project=$4,renew_project_id=$5,action_id=$6 WHERE id=$7`,
		r.CommissionID, r.Value, r.Comment, r.Project, r.RenewProjectID, r.ActionID, r.ID)
	if err != nil {
		return err
	}
...
func (r *RenewProjectForecasts) Get(ID int64, db *sql.DB) (err error) {
	...
	r.project,r.comment,r.renew_project_id, r.action_id, b.code, b.name
	FROM renew_project_forecast r
	JOIN commission c ON c.id=r.commission_id
	JOIN budget_action b ON b.id=r.action_id
	...
	for rows.Next() {
		if err = rows.Scan(&row.ID, &row.CommissionID, &row.CommissionDate,
			&row.CommissionName, &row.Value, &row.Project, &row.Comment,
			&row.RenewProjectID, &row.ActionID, &row.ActionCode, &row.ActionName); err != nil {
			return err
		}
...
func (r *RenewProjectForecasts) GetAll(db *sql.DB) (err error) {
	...
	for rows.Next() {
		if err = rows.Scan(&row.ID, &row.CommissionID, &row.CommissionDate,
			&row.CommissionName, &row.Value, &row.Project, &row.Comment, &row.RenewProjectID,
...
func (r *RenewProjectForecastBatch) Save(db *sql.DB) (err error) {
	...
	stmt, err := tx.Prepare(pq.CopyIn("temp_renew_project_forecast", "id",
		"commission_id", "value", "project", "comment", "renew_project_id", "action_code"))
	if err != nil {
		return err
	}
	defer stmt.Close()
	for _, r := range r.Lines {
		if _, err = stmt.Exec(r.ID, r.CommissionID, r.Value, r.Project, r.Comment,
	...
	value=t.value,comment=t.comment,project=t.project,renew_project_id=t.renew_project_id,
	action_id=b.id
	FROM temp_renew_project_forecast t, budget_action b 
	WHERE t.id = renew_project_forecast.id AND t.action_code=b.code`,
		`INSERT INTO renew_project_forecast (commission_id,value,project,comment,
			renew_project_id,action_id)
	SELECT t.commission_id,t.value,t.project,t.comment,t.renew_project_id, b.id
