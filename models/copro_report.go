...
func (c *CoproReports) GetAll(db *sql.DB) error {
	qry := `
  WITH max_cmt_date AS (SELECT MAX(creation_date) AS d FROM commitment)
  SELECT ci.insee_code,ci.name,co.name,co.budget,cmt.value,prg.value,
    q.y1,q.y2,q.y3,q.y4,q.y5 FROM
		(SELECT * FROM
			crosstab(
				'SELECT cf.id,EXTRACT(year FROM c.date),cf.value from copro_forecast cf
		...
		LEFT OUTER JOIN (SELECT SUM(value)::bigint AS value,kind_id AS copro_id FROM prog 
			...
			WHERE kind=2 AND co.date>(SELECT d FROM max_cmt_date) GROUP BY 2) prg 
	...
	var l CoproReport
	for rows.Next() {
		if err = rows.Scan(&l.InseeCode, &l.CityName, &l.CoproName, &l.Budget,
